...
@section('contents')
    ...
    <section class="section-box-2">
        <div class="container">
            <div class="banner-hero banner-image-single"><img style="height: 374px; object-fit: cover;" src="{{ asset($company?->banner) }}" alt="Company Banner"></div>
            <div class="box-company-profile">
                <div class="row mt-10">
                    <div class="col-lg-8 col-md-12">
                        <div>
                            <img style="height: 100px; width: 100px; object-fit: cover; border-radius: 50%" src="{{ asset($company?->logo) }}" alt="">
                        </div>
                        <h5 class="f-18">{{ $company?->name }} <span class="card-location font-regular ml-20">{{ $company?->countryName?->name }}</span></h5>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-md-12 col-sm-12 col-12">
                    ...
                    <div class="box-related-job content-page" id="openJobs">
                        ...
                        <div class="sidebar-heading">
                            <div class="avatar-sidebar">
                                <div class="sidebar-info pl-0"><span class="sidebar-company">{{ $company?->name }}</span><span
                                        class="card-location">{{ $company?->countryName?->name }}</span></div>
                            </div>
                        </div>
                        ...
                        <div class="sidebar-list-job">
                            <ul>
                                <li>
                                    ...
                                    <div class="sidebar-text-info"><span class="text-description">Industry Type</span><strong class="small-heading">{{ $company?->industryType?->name }}</strong></div>
                                </li>
                                <li>
                                    <div class="sidebar-icon-item"><i class="fi-rr-marker"></i></div>
                                    <div class="sidebar-text-info"><span class="text-description">Organization Type</span><strong class="small-heading">{{ $company?->organizationType?->name }}</strong></div>
                                </li>
                                <li>
                                    <div class="sidebar-icon-item"><i class="fi fi-rr-user"></i></div>
                                    <div class="sidebar-text-info"><span class="text-description">Team Size</span><strong class="small-heading">{{ $company?->teamSize?->name }}</strong></div>
                                </li>
                                <li>
                                    <div class="sidebar-icon-item"><i class="fi-rr-clock"></i></div>
                                    <div class="sidebar-text-info"><span class="text-description">Establishment Date</span><strong class="small-heading">{{ formatDate($company?->establishment_date) }}</strong></div>
                                </li>
                            </ul>
                        </div>
                        <div class="sidebar-list-job">
                            <ul class="ul-disc">
                                <li>Address: {{ $company?->address }} {{ $company?->cityName?->name ? ', '.$company?->cityName?->name : '' }} {{ $company?->stateName?->name ? ', '.$company?->stateName?->name : '' }} {{ $company?->countryName?->name ? ', '.$company?->countryName?->name : '' }}</li>
                                <li>Phone: {{ $company?->phone }}</li>
                                <li>Email: {{ $company?->email }}</li>
                                <li>Website: <a href="{{ $company?->email }}">{{ $company?->email }}</a></li>
                            </ul>
